...
export default class TournamentController {
    ...
    static joinTournament = async (req, res) => {
    ...
    static getTournamentData = async (req, res) => {
        try {
    ...
    static getJoinTournamentData = async (req, res) => {
        try {
            const page = req.query.page;
    ...
    static getContract = async (req, res) => {
        try {
            ...
            const rawdata = await readFile(contractLocation);
    ...
    static updateTournament = async (name, player) => {
        ...
        if (tournament.currentGames.length !== 1 && (tournament.nextRound.length === tournament.currentGames.length || tournament.nextRound.length === tournament.numberOfPlayers)) {
            ...
            for (i = 0; i < matches.length; i++) {
                matches[i] = await GameController.createTournamentGame(matches[i], name);
    ...
    static updateContractStatus = async (tournamentId) => {
    ...
    static updatePlayersList = async (tournamentId, playerAddress) => {
        ...
        tournament
            .save()
            .then(tournament => {
                UserController.addUserTournament({
    ...
    static matchGamePlayers = async (nextRound) => {
        const matches = [];
